...
def Net(): #定义函数，用于创建适用于CIFAR-10的ResNet8模型。它加载一个预训练的ResNet8模型，并将最后的全连接层替换为适用于CIFAR-10的层
    num_classes = 10
...
def getDataset(): #创建一个带有制定转换的CIFAR-10数据集
...
def basic_loader(num_clients, loader_type): #基于提供的加载器类型（iid、non-iid、dirichlet）和客户端数量创建一个基本数据加载器
    dataset = getDataset()
    return loader_type(num_clients, dataset)


def train_dataloader(num_clients, loader_type='iid', store=True, path='./data/loader.pk'): #创建并存储一个数据加载器。
    #它首先检查是否存在存储的加载器；如果不存在，则根据指定的加载器类型初始化一个。加载器可以使用 pickle 序列化进行存储。返回数据加载器。
...
def test_dataloader(test_batch_size): #为CIFAR-10数据集创建一个测试数据加载器
